...
class Controller:
    ...
    def tour_encours(self):
        ...
        tour_encours = Tour(q['idtournoi'], q['nom'], q['matchs'], q['idtour'], q['date_debut'],
                            q['date_fin'], q['etat'])
        return tour_encours
    ...
    def get_liste_joueurs(self):
        ...
        for j in tournoi_encours.joueurs:
            ...
            joueur = Joueur(q['nom'], q['prenom'], q['date_naissance'], q['sexe'], q['classement'], q['points'],
                            q['idjoueur'])
    ...
    def creer_matchs_tour(self, paires_tour):
        ...
        for p in paires_tour:
            try:
                idmatch = max(list(map(lambda x: x['idmatch'], self.db.get_all('match')))) + 1
            except ValueError:
                idmatch = 1
    ...
    def get_next_paires(self, tournoi):
        ...
        liste_joueurs = self.get_liste_joueurs()  # liste des instances de joueurs triée
        nb_joueurs = len(liste_joueurs)
        ...
        for i in range(0, nb_joueurs - 1):
            ...
            if paire in matchs_joues or paire_reverse in matchs_joues:
                pass
            else:
                paires_tour.append(paire)
                lj.extend(paire)
        # ces règles ont elles permis de générer toutes les paires attendues ?
        nb_paires_manquantes = int(tournoi.nbtours - len(paires_tour))
        # Si non : identification des combinaisons manquantes
        if nb_paires_manquantes > 0:
            for i in range(0, nb_paires_manquantes):
                manquants = []
                for j in tournoi.joueurs:
                    if j not in lj:
                        ...
                        pass
                combinaisons = list(map(lambda x: list(x), list(combinations(manquants, 2))))
                non_joues = [c for c in combinaisons if c not in matchs_joues]
                paire = non_joues[0]
                paires_tour.append(paire)
                lj.extend(paire)
        else:
    ...
    def demarrer_tour(self):
        ...
        if nb_joueurs == tournoi_encours.nbtours*2:
            if self.db.query_3('MATCH', 'idtour', self.tour_encours().idtour, 'score1', 0, 'score2', 0):
    ...
    def entrer_resultats_tour(self):
        ...
        for m in list_matchs:
            ...
            player1 = Joueur(j1['nom'], j1['prenom'], j1['date_naissance'], j1['sexe'], j1['classement'],
                             j1['points'], j1['idjoueur'])
            player2 = Joueur(j2['nom'], j2['prenom'], j2['date_naissance'], j2['sexe'], j2['classement'],
                             j2['points'], j2['idjoueur'])
        ...
        self.db.update_item('TOUR', 'date_fin', tour_encours.date_fin, 'idtour', tour_encours.idtour)
        ...
        if indice_tour <= tournoi_encours.nbtours:
            ...
            idtour = str(tournoi_encours.idtournoi) + 'T' + str(indice_tour)
            matchs = []
            newtour = Tour(tournoi_encours.idtournoi, nom_tour, matchs, idtour)
            self.db.insert(newtour)
            tournoi_encours.addTour(newtour)
            self.db.update_item('TOURNOI', 'tours', tournoi_encours.tours, 'idtournoi', tournoi_encours.idtournoi)
    ...
    def maj_classement(self):
        ...
        try:
            j_id = self.db.query_2('JOUEUR', 'nom', lastname, 'prenom', firstname)[0].get('idjoueur')
    ...
    def get_all_tournois(self):
        ...
        print("\n".join(list(map(lambda x: str(x['idtournoi']) + " - " + x['nom'], self.db.get_all('tournoi')))))
    ...
    def get_all_matchs_tournoi(self, idtournoi):
        ...
        q = list(map(lambda x: x['idtour'] + " " + str(x['idmatch']) + " - Joueur " + str(x['joueur1']) +
                     " vs " + str(x['joueur2']) + " Score: " + str(x['score1']) + "/" + str(x['score2']),
    ...
    def classement_general(self):
        ...
        print("\n".join(list(map(lambda x: " n°:" + str(x.classement) + " " + x.nom + " " + x.prenom +
                                           " (id: " + str(x.idjoueur) + ")", liste))))
    ...
    def classement_tournoi(self, idtournoi):
        ...
        print("\n".join(list(map(lambda x: "Points: " + str(x.points) + " - " + x.nom + " " + x.prenom, liste))))
